"""Validation of the parameters accepted by the media discovery and search tools.

These checks depend on nothing but the caller's arguments: no provider, no
network, no MCP server. They apply to any media type the tools grow to cover.
"""
...
# Bounds enforced on the parameters callers supply. The max_results bounds are
# result-count policy shared with the service layer, so they live in media_limits.
MIN_YEAR = 1900
MIN_PAGE = 1
...
def _validate_max_results(max_results: int) -> None:
    """Reject result counts outside the supported page size."""
